...
<div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden">
    <!-- En-tête -->
    <div class="px-5 py-3 from-gray-600 to-primary-700 dark:from-gray-700 dark:to-gray-800">
        <div class="flex items-center justify-between">
            <div class="flex-1">
                <h2 class="text-lg font-bold font-heading text-gray-800 dark:text-primary-100 flex items-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                    Saisie des Notes
                    @if($ecSelected)
                        <span class="text-yellow-500 text-sm font-body">({{ $ecSelected->abr }}-{{ $ecSelected->nom }})</span>
                    @endif
                </h2>
                <div class="flex items-center gap-3 mt-1 text-xs font-body text-gray-800 dark:text-primary-100">
                    @if($niveauSelected)
        ...
        @else
            ...
            <div class="max-w-4xl mx-auto">
                ...
                @if($totalCopies > 0)
                    <div class="mb-4 p-3 rounded-lg border border-gray-200 dark:border-gray-800">
                        ...
                        <div class="mt-1 flex justify-between text-xxs font-body text-primary-600 dark:text-primary-400">
                            <span class="font-bold">{{ $progressCount }} saisies</span>
                            <span class="font-bold">{{ $this->copiesRestantes }} restantes</span>
                        </div>
                    </div>
                @endif

                <!-- ÉTAPE 1: Champ d'identification -->
                <div class="p-4 rounded-lg border border-gray-200 dark:border-gray-700 mb-4">
                    @if($is_active)
                        <!-- Mode Matricule -->
                        <div class="space-y-3">
                    ...
                    @else
                        <!-- Mode Code Anonymat avec préfixe automatique -->
                        <div class="space-y-3">
                            <label for="numeroCode" class="flex items-center gap-1.5 text-sm font-semibold font-heading text-gray-900 dark:text-white">
                                <svg class="w-4 h-4 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="relative flex items-stretch">
                                <!-- Préfixe affiché - TAILLE AUGMENTÉE -->
                                <div class="flex items-center px-5 bg-gradient-to-r from-green-100 to-green-50 dark:from-green-900/40 dark:to-green-900/20 border-2 border-r-0 border-green-400 dark:border-green-500 rounded-l-lg shadow-sm">
                                    <span class="text-2xl font-extrabold font-mono text-green-700 dark:text-green-300 tracking-wider">
                                        {{ $this->codeSalle }}
                                    </span>
                                </div>
                                
                                <!-- Input pour le numéro uniquement - TAILLE AUGMENTÉE -->
                                <input 
                                    type="number" 
                                    id="numeroCode"
                                    wire:model.live="numeroCode"
                                    placeholder="1, 2, 3..."
                                    autocomplete="off"
                                    min="1"
                                    step="1"
                                    class="flex-1 px-4 py-3 text-xl font-bold font-mono rounded-r-lg border-2 border-l-0 transition-all duration-200
                                    @if($codeAnonymatCourant && !$noteDejaExiste) 
                                        border-green-500 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300
                                    @elseif($noteDejaExiste) 
                                        border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-300
                                    @elseif($numeroCode && strlen($numeroCode) >= 1 && !$codeAnonymatCourant) 
                                        border-red-500 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300
                                    @else 
                                        border-primary-400 dark:border-primary-500 focus:border-primary-600 focus:ring-2 focus:ring-primary-500
                                    @endif
                                    bg-white dark:bg-gray-800 text-gray-900 dark:text-white
                                    placeholder:text-gray-400"
                                />
                                ...
                                @elseif($noteDejaExiste || ($numeroCode && strlen($numeroCode) >= 1 && !$codeAnonymatCourant))
                                    <div class="absolute right-3 top-1/2 -translate-y-1/2">
                            ...
                            @elseif($numeroCode && strlen($numeroCode) >= 1 && !$codeAnonymatCourant && !$noteDejaExiste)
                                <div class="mt-2 p-3 bg-red-50 dark:bg-red-900/20 border-l-3 border-red-500 rounded-r-lg">
                                    <div class="flex items-start gap-2">
                                        <svg class="w-5 h-5 text-red-600 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/>
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <p class="text-xs font-body text-red-800 dark:text-red-400 mt-0.5">
                                                Le code "{{ $codeAnonymat }}" n'existe pas pour cette EC
                                            </p>
                                        </div>
                                    </div>
                                </div>
                ...
                @if($afficherChampNote && !$noteDejaExiste)
                    <div class="p-4 rounded-lg border-2 border-green-300 dark:bg-green-900/20">
                        ...
                        <div class="flex items-center gap-3">
                            <input 
                                ...
                                class="flex-1 px-4 py-2.5 text-lg font-bold font-heading rounded-lg border-2 transition-all duration-200
                                    ...
                                    @else border-green-300 dark:border-green-600 bg-white dark:bg-gray-800 focus:border-green-500 focus:ring-2 focus:ring-green-500 @endif
    ...
    @if($progressCount > 0 || $totalCopies > 0)
        <div class="px-5 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
            ...
            @if($this->intervalleCodes['min'] && $this->intervalleCodes['max'])
                <div class="mb-3 p-2.5 border border-blue-200 dark:border-blue-700 rounded-lg">
