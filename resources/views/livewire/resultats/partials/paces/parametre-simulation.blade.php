...
<div x-data="{ open: @entangle('simulationEnCours') }"
    ...
    @php
        // Utiliser d'abord les brouillons s'ils existent
        $draftQuota = $draft['quota_admission'] ?? $quota_admission;
        $draftCr    = $draft['credits_requis'] ?? $credits_requis;
        $draftMr    = $draft['moyenne_requise'] ?? $moyenne_requise;

        // Cast sûrs
        $cr = is_numeric($draftCr) ? (float)$draftCr : 0.0;
        $mr = is_numeric($draftMr) ? (float)$draftMr : 0.0;

        // Pourcentages bornés
        $crPct = min(100, max(0, ($cr / 60) * 100));
        $mrPct = min(100, max(0, ($mr / 20) * 100));
    @endphp
    ...
    <button @click="open = !open"
        ...
        <div class="flex items-center gap-3 flex-1">
            ...
            <div class="flex items-center gap-2 flex-1">
                <h3 class="text-base font-bold text-gray-900 dark:text-gray-100">Paramètres de Délibération</h3>
        ...
        <div class="px-5 pb-4 pt-1 border-t border-gray-100/50 dark:border-gray-700/50">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                ...
                <div class="group">
                    ...
                    <input type="number"
                           wire:model.defer="draft.quota_admission"
                           min="0"
                           placeholder="∞"
                    ...
                    @if($derniereDeliberation && ($draftQuota !== $derniereDeliberation->quota_admission))
                ...
                <div class="group">
                    ...
                    <div class="relative">
                        <input type="number"
                               wire:model.defer="draft.credits_requis"
                               min="0" max="60"
                ...
                <div class="group">
                    ...
                    <div class="relative">
                        <input type="number"
                               wire:model.defer="draft.moyenne_requise"
                    ...
                    <div class="mt-1 w-full bg-gray-200/50 dark:bg-gray-700/50 rounded-full h-0.5 overflow-hidden">
                        <div class="bg-gradient-to-r from-amber-500 to-amber-600 h-0.5 rounded-full transition-all"
                             style="width: {{ $mrPct }}%"></div>
                    </div>
                </div>

                {{-- 4️⃣ Note éliminatoire --}}
                <div class="group flex flex-col justify-center">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700/50 hover:border-gray-300 dark:hover:border-gray-500 transition-all">
                        <input type="checkbox"
                               wire:model.defer="draft.appliquer_note_eliminatoire"
