...
<script>
    window.chartData = {
        tauxReussite: @json($chartDataTauxReussite ?? array_fill(0, 12, 0)),
        admis: @json($chartDataAdmis ?? array_fill(0, 12, 0)),
        moyennes: @json($chartDataMoyennes ?? array_fill(0, 12, 10)),
        etudiantsAdmis: {{ $etudiantsAdmis ?? 0 }},
        rattrapage: {{ $rattrapage ?? 0 }},
        redoublants: {{ $redoublants ?? 0 }},
        exclus: {{ $exclus ?? 0 }}
    };
